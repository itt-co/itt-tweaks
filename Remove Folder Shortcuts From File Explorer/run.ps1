    ...
    foreach ($Item in $Registry) {
        if ($Item.Remove -eq $true) {
            if (Test-Path $Item.Path) {
                ...
                Write-Host "[INFO] Removed $($Item.Path)" -ForegroundColor Cyan
            } else {
                Write-Host "[INFO] Not Found (Already Removed) $($Item.Path)" -ForegroundColor Cyan
            }
        } else {
            ...
            Write-Host "[INFO] Set $($Item.Path)\$($Item.Name) = $($Item.Value)" -ForegroundColor Cyan
        }
    }
} catch {
    Write-Host "[!] Error: $_"
